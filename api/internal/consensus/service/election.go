...
func (n *Node) RequestVote(ctx context.Context, req *pb.VoteRequest) (*pb.VoteResponse, error) {
	...
	if !req.Gossip && (n.state.Role() == core.Leader || n.state.Role() == core.Candidate) && req.Term == n.state.CurrentTerm() {
	...
	if req.Gossip && n.state.LeaderID() != "" && !n.state.IsElectionTimeout() {
