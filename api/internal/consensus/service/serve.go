...
func (n *Node) ClientAppend(ctx context.Context, req *pb.RawAppendRequest) (*pb.AppendResponse, error) {
	...
	stdResp := &pb.AppendResponse{
		...
		Status:        pb.AppendResponseStatus_APPEND_RESPONSE_BAD_PAYLOAD,
		PrevLogTerm:   -1,
		PrevLogIndex:  -1,
	...
	glog.V(3).Infof("Append request from a client received for SchedulerID: %s, RequestID: %s", req.SchedulerId, req.RequestId)

	// Delegate to the helper to check for Thundering Herd race conditions
	cas := n.checkCASConflict(req.Payload)

	switch cas {
	case casNoConflict:
		// Explicitly break
	case casDuplicateSchedulerEvent:
		stdResp.Status = pb.AppendResponseStatus_APPEND_RESPONSE_BAD_PAYLOAD
		return stdResp, nil
	case casConcurrentClaim:
		stdResp.Status = pb.AppendResponseStatus_APPEND_RESPONSE_CONCURRENT_CLAIM
		return stdResp, nil
	default:
		return nil, fmt.Errorf("unknown CAS conflict type: %v", cas)
	}
	...
	case resp := <-ch:
		if resp {
			stdResp.Status = pb.AppendResponseStatus_APPEND_RESPONSE_SUCCESS
		} else {
			// Return Status 3 (Consensus Failure) if replication failed to achieve quorum.
			stdResp.Status = pb.AppendResponseStatus_APPEND_RESPONSE_FAILED_CONSENSUS
		}
		return stdResp, nil
	}
}

type casConflict int

const (
	casNoConflict casConflict = iota
	casConcurrentClaim
	casDuplicateSchedulerEvent
)

// checkCASConflict inspects the incoming payload against the current authoritative log to prevent duplicates
func (n *Node) checkCASConflict(payload []byte) casConflict {
	...
	if err := json.Unmarshal(payload, &incoming); err != nil || incoming.ID == "" {
		return casNoConflict // Not a parsable or tracked payload, let it through
	}
	...
	if !isJobClaim && !isSchedulerEvent {
		return casNoConflict // Nothing to guard against
	}

	existing := n.FileHandler.QueryEntries(nil, &incoming.ID)
	if len(existing) == 0 {
		return casNoConflict
	}
	...
	if isSchedulerEvent && current.Entry != nil {
		...
		if isThunderingHerdEvent && inStr == curStr {
			...
			if timeDiffMillis > -5000 && timeDiffMillis < 5000 {
				...
				return casDuplicateSchedulerEvent
			}
		}
	}
...
func (n *Node) GetData(ctx context.Context, req *pb.GetDataRequest) (*pb.GetDataResponse, error) {
	if req.SchedulerIdQuery != nil && req.PayloadIdQuery != nil {
		glog.V(2).Infof("GetData: Received query for SchedulerID: %s, PayloadID: %s", *req.SchedulerIdQuery, *req.PayloadIdQuery)
	} else {
		glog.V(2).Infof("GetData: Received query where one or more fields is nil")
	}
