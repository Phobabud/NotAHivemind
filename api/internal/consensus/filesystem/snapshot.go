...
func (h *Handler) InstallSnapshot(offset int64, data []byte, done bool, lastIncludedIndex int64, lastIncludedTerm int64) error {
	...
	if _, err := file.WriteAt(data, offset); err != nil {
		file.Close() // Explicitly clean up on error
	...
	if done {
		// Ensure the destination (active.log) is also fully unlocked before we try to overwrite it
		if h.snapshotFile != nil {
			_ = h.snapshotFile.Close()
			h.snapshotFile = nil
		}
		...
		// Reopen the newly moved active.log so the node can read from it
		h.snapshotFile, err = os.OpenFile(snapPath, os.O_RDONLY, 0644)
		if err != nil {
			return fmt.Errorf("failed to reopen snapshot file: %w", err)
		}

		// Truncate the old WAL (active log) since the snapshot now contains the entire state
		if h.activeFile != nil {
