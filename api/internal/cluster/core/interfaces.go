...
type ContainerPool interface {
	...
	*/
	...
	FindFreeContainer(ctx context.Context, imageName string) (ContainerAPI, error)
...
type JobQueue interface {
	...
	*/
	...
	// AddJob takes a job pointer, and adds it to the pending jobs list
	AddJob(job *Job) error

	// RemoveJob takes a job's ID, and removes it from the pending jobs list
	RemoveJob(id string) error

	// FindJob takes a job's ID and finds it in the pending jobs list
	FindJob(id string) (*Job, error)

	// PendingJobs creates a snapshot of the pending jobs array
	PendingJobs() []*Job

	// ActiveJobs creates a snapshot of the active jobs array
	ActiveJobs() []*Job
	...
	*/
	...
	// MarkRunning will take a pre-running job and move it to be active
	MarkRunning(id string, containerID string) error

	// RollbackJob will take an active or pre-running job and move it back to pending
	...
	*/
	...
	QueueUtilization() (int64, int64)
	ActiveUtilization() (int64, int64)
}
