...
func (c *Container) WaitUntilReady(ctx context.Context, timeout time.Duration) error {
	...
	for {
		...
		if time.Now().After(deadline) {
			return fmt.Errorf("timed out waiting for container %s to be ready", c.id)
		}
...
func processAndDeleteFile(filePath string) (json.RawMessage, error) {
	fileBytes, err := readWithRetry(filePath)
	if err != nil {
		return nil, fmt.Errorf("failed to open and read file: %w", err)
	}
...
func readWithRetry(path string) ([]byte, error) {
	...
	return nil, fmt.Errorf("failed to read file after retries: %v", lastErr)
}
