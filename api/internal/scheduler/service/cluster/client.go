...
func (c *Conn) DispatchJob(ctx context.Context, job *core.Job) (bool, error) {
	...
	req := &pb.JobRequest{
		JobId:               job.Id,
		RequiredNanoCpu:     job.CPURequirement,
		RequiredMemoryBytes: job.MemoryRequirement,
...
func (c *Conn) monitorJobStatuses(ctx context.Context) {
	for {
		...
		// Don't spam network until connection happens
		if !c.Connection() {
			select {
			case <-ctx.Done():
				return
		...
		stream, err := c.client.ListJobs(ctx, &emptypb.Empty{})
		if err != nil {
		...
		for {
			...
			if event.Status == models.Completed || event.Status == models.Failed {
				err := c.state.CompleteJob(event.JobID, event.Payload, event.Status == models.Completed)
				if err != nil {
