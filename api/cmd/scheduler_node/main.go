...
import (
	"NotAHiveMind/internal/scheduler/config"
	"NotAHiveMind/internal/scheduler/packer"
...
//go:generate protoc -I=../../ --go_out=../../ --go_opt=module=NotAHiveMind --go-grpc_out=../../ --go-grpc_opt=module=NotAHiveMind ../../api/proto/scheduling/v1/coordinate.proto
...
func init() {
	configFile := flag.String("config", "test_config.json", "Path to the scheduler-example configuration file")
	flag.Parse()
...
func main() {
	...
	go state.ClearCompletedJobs(ctx)
