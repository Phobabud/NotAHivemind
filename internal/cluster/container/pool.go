...
import (
	core2 "ClusterManager/internal/cluster/core"
...
func (c *Containers) Add(targetContainer core2.ContainerAPI) error {
	c.mutex.Lock()
...
func (c *Containers) AddFromImage(image *core2.Image) (core2.ContainerAPI, error) {
...
func (c *Containers) Remove(id string) error {
	c.mutex.Lock()
	defer c.mutex.Unlock()
...
func (c *Containers) Get(id string) (core2.ContainerAPI, error) {
	c.mutex.Lock()
	defer c.mutex.Unlock()
	for _, container := range c.Containers {
		if container.id == id {
...
func (c *Containers) FindFreeContainer(imageName string) (core2.ContainerAPI, error) {
	c.mutex.Lock()
	defer c.mutex.Unlock()
	matchingImage := false
	allOccupied := true
	for _, container := range c.Containers {
		if !container.occupied && container.image.Alias == imageName {
			return container, nil
		}
	...
	if !matchingImage {
		return nil, core2.ErrImageNotFound
	}
	if allOccupied {
		return nil, core2.ErrAllOccupied
	}
...
func (c *Containers) AvailableImageNames(ctx context.Context) []string {
	...
	for _, container := range c.Containers {
		...
		if status, err := container.Health(ctx); !container.occupied && status == 0 && err == nil {
			lst = append(lst, container.image.Alias)
		}
	}
	return lst
...
func (c *Containers) ForEach(action func(container core2.ContainerAPI) error) error {
	c.mutex.Lock()
	defer c.mutex.Unlock()
	for _, container := range c.Containers {
...
func (c *Containers) UsedSpace() (int64, int64) {
	...
	memoryTotal := int64(0)
	for _, container := range c.Containers {
		nanoCpuTotal += container.cpuLimit
