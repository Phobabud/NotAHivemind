...
type Container struct {
	...
	mutex            *sync.RWMutex
	totalJobs        int
	lastAssignedJob  time.Time
	lastCompletedJob time.Time
	occupied         bool
...
func NewContainer(cli *client.Client, logger *filehandler.LogWriter, image *core.Image, payloadVolumeLocation string, port string) core.ContainerAPI {
	return &Container{
		client:           cli,
		id:               fmt.Sprintf("%s-%d", image.Alias, time.Now().UnixNano()),
		logger:           logger,
		image:            image,
		...
		memLimit:         image.MemoryLimit,
		localPort:        normalizeHTTPPort(port),
		totalJobs:        0,
		lastAssignedJob:  time.Now(),
		lastCompletedJob: time.Now(),
		occupied:         false,
		mutex:            &sync.RWMutex{},
	}
}
